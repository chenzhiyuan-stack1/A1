...
from utilities import imitation_terminal_conditions
...
class ImitationTask(object):
  ...
  def build_target_obs(self):
    ...
    tmp = self._env.sim.data.qpos.ravel().copy()
    sim_base_rot = [tmp[4],tmp[5],tmp[6],tmp[3]]
  ...
  def _calc_reward_pose(self):
    """Get the pose reward."""
    env = self._env
    sim_model = self._env.sim
    ref_model = self._env.ref_sim
  ...
  def _calc_reward_end_effector(self):
    ...
    for j in range(4):
      foot_link_name = "foot" + str(j+1)
      end_pos_ref = env.ref_data.get_geom_xpos(foot_link_name)
      end_pos_sim = env.data.get_geom_xpos(foot_link_name)
  ...
  def _sync_sim_model(self, perturb_state):
    ...
    self._set_state(self._env.sim , pose, vel)
    return
  ...
  def _set_state(self, phys_model, pose, vel):
    ...
    # 上面构造的qpos和qvel都要是.ravel()过的，只有一个维度
    # pyhsics相当于sim
    qpos = np.array([qpos,])
    qvel = np.array([qvel,])
    old_state = phys_model.get_state()
    new_state = mujoco_py.MjSimState(old_state.time, qpos, qvel,
                                         old_state.act, old_state.udd_state)
    phys_model.set_state(new_state)
    phys_model.forward()
    return
  ...
  def _get_sim_base_position(self):
    ...
    # pos = np.array(pos)
    pos = self._env.sim.data.qpos.ravel()[0:3]
    return pos
  ...
  def _get_sim_base_rotation(self):
    ...
    tmp = self._env.sim.data.qpos.ravel()
    rotation = [tmp[4],tmp[5],tmp[6],tmp[3]]
    return rotation
  ...
  def _get_ref_base_position(self):
    ...
    # pos = np.array(pos)
    pos = self._env.ref_sim.data.qpos.ravel()[0:3]
    return pos
  ...
  def _get_ref_base_rotation(self):
    ...
    # rotation = np.array(rotation)
    tmp = self._env.ref_sim.data.qpos.ravel()
    rotation = [tmp[4],tmp[5],tmp[6],tmp[3]]
    return rotation
  ...
  def _build_sim_pose(self, phys_model):
    ...
    tmp = phys_model.data.qpos.ravel()
    root_rot = [tmp[4],tmp[5],tmp[6],tmp[3]]
